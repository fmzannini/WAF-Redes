#!/usr/bin/env bash

#echo -e "-- BEGIN ${HOSTNAME} --\n"
#echo -e "installing node"
#sudo apt-get install python-software-properties
#curl -sL https://deb.nodesource.com/setup_7.x | sudo -E bash -
#sudo apt-get install nodejs

# postgres
#sudo apt-get update -y

if ! [ -d ~/.rbenv/plugins/ruby-build ]; then

vagrant@webserver1:~$ /var/lib/postgresql/9.3/main/^C

fi

#sudo apt-get install libpq-dev postgresql-common postgresql -y
sql_command="ALTER ROLE \"postgres\" WITH LOGIN SUPERUSER PASSWORD 'postgres';"
sudo sql_command="$sql_command" -Eu postgres bash -c 'psql -c "$sql_command"'
sudo db_command="CREATE DATABASE mydb;" -Eu postgres bash -c 'psql -c "$db_command"'
sudo db_command="CREATE TABLE notes(note varchar(300));" -Eu postgres bash -c 'psql -d mydb -c "$db_command"'
sudo db_command="SELECT * FROM notes;" -Eu postgres bash -c 'psql -d mydb -c "$db_command"'

# https://stackoverflow.com/questions/18664074/getting-error-peer-authentication-failed-for-user-postgres-when-trying-to-ge

# sequelize
#npm i -g pg
#npm i -g sequelize
#npm i pg
#npm i sequelize

# server
node server.js
